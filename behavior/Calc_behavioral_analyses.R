...
dataResp$response = as.factor(toupper(dataResp$response))
...
plot_data_add <- ggplot(data_plot, aes(x=Congruency, y=mean,fill=Congruency, width=.5)) + geom_bar(position ='identity' , stat='identity') + geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position=position_dodge(.5),width=.01) + scale_fill_manual(values=c("grey", "grey","grey","grey","grey")) + labs(y = "RT") + labs(x = "Deviant") + theme_bw() + theme(legend.position="none") + scale_y_continuous(limits=c(0,1200),breaks=seq(0, 1500, 100)) + theme(text = element_text(size=20)) + ggtitle(sprintf("Addition %s", subjname)) 
ggsave(file=sprintf("plot_addition_%s.eps", subjname), plot= plot_data_add, width=6, height=6)
...
plot_data_sub <- ggplot(data_plot, aes(x=Congruency, y=mean,fill=Congruency, width=.5)) + geom_bar(position ='identity' , stat='identity') + geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position=position_dodge(.5),width=.01) + scale_fill_manual(values=c("grey", "grey","grey","grey","grey")) + labs(y = "RT") + labs(x = "Deviant") + theme_bw() + theme(legend.position="none") + scale_y_continuous(limits=c(0,1200),breaks=seq(0, 1500, 100)) + theme(text = element_text(size=20)) + ggtitle(sprintf("Subtractions %s", subjname)) 
ggsave(file=sprintf("plot_subtractions_%s.eps", subjname), plot= plot_data_sub, width=6, height=6)
...
plot_data_comp <- ggplot(data_plot, aes(x=Congruency, y=mean,fill=Congruency, width=.5)) + geom_bar(position ='identity' , stat='identity') + geom_errorbar(aes(ymin=mean-se,ymax=mean+se),position=position_dodge(.5),width=.01) + scale_fill_manual(values=c("grey", "grey","grey","grey","grey")) + labs(y = "RT") + labs(x = "Deviant") + theme_bw() + theme(legend.position="none") + scale_y_continuous(limits=c(0,1200),breaks=seq(0, 1500, 100)) + theme(text = element_text(size=20)) + ggtitle(sprintf("Comparisons %s", subjname)) 
ggsave(file=sprintf("plot_comparisons_%s.eps", subjname), plot= plot_data_comp, width=6, height=6)
