...
- sub answers are coded as follow to identify later both YES vs NO and LEFT vs RIGHT click
            YES = 2 on LEFT and NO = 1 on RIGHT
            YES = 4 on RIGHT and NO = 3 on LEFT
- save results (one sub = one folder, 8 runs = 8 files)
...
- number of frames computed for monitor with 60hz refresh rate (16.7 ms per frame)
    ...
    stim dur tot range:
        from 132  frames to 168 frames
            of which
                    stimulus   300 ms =   frames => 18
                    min ISI    2171 ms =  frames => 130
                    max ISI    3340 ms =  frames => 200
    odd dur tot:
            3340 ms = frames   => 200
            of which 
                    odd  100
...
- 10 odd * 8 runs = 36   => 40 yes and 40 no
====================================================
'''
...
def PresenttargetResp(eventStim,ISI,eventDelay):  
    event.clearEvents()
    resp_event = 0
    rt_event = 0
    
    if eventDelay == 0:
        code_event = int(eventStim+31)
    else:
        code_event = int(eventStim+41)    
    ...
    for frameN in range(tot_trial):
        ...
        if 1 <= frameN < 5: # duration of the stimulus
            ...
            win.flip()
            if resp_event == 0:
                resp_event, rt_event = WaitTyesnoAnswer(win, resp_event,rt_event,onset_event)
        if 5 <= frameN < StimDur: # duration of the stimulus
            fixation.draw()
            stim.draw()
            win.flip()
    ...
    # for i in range(0,ntrials):
        ...
        for e in range(0, len(trial)-1): # exclude the last column which is the info of the delay
            ...
            if e == 0:
                ...
                eventStim = trial[4]
                resp_event, rt_event, onset_event, code_event = PresenttargetResp(eventStim, ISI, eventDelay)
...
pylink.getEYELINK().receiveDataFile(edfFileName,resultpath + '/' + edfFileName)
