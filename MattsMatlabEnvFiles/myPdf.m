function [PdfOut,Centers] = myPdf(Data,BinWidth,useZeroMin,graphflag,useCA)
%
% Begun on 060403 by Matt Nelson to makes a quick pdf of the data input to
...
%           Note- you can also enter a vector if you wish to specify the
%           center of each individual bin instead of the bin width.
...
if ~iscell(Data)
    ...
    if c>r  %then put each row of Data into a cell
        temp=mat2cell(Data,ones(1,r),c);
    else    %then put each column of Data into a cell
        temp=mat2cell(Data,r,ones(1,c));
    end
    clear Data;
    Data=temp;
end
