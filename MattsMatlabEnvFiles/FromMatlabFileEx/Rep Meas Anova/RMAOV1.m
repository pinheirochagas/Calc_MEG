function [p F Adj_df eps] = RMAOV1(X)
...
%              independent variable 1=column 2; subject=column 3).
%   
%   Outputs:
%            - p: a 1x2 vector array of p-vals for: main effect A and 
%            subject effect (technically the main subject effect should not
%            be included...)
%            - F: a 1x2 vector of F-statistics corresponding to the same
%            tests to give the p-values
%            - Adj_df: a 2x2 array of the adjusted degrees of freedom for 
...
disp('Running repeated measures ANOVA- RMAOV1')
...
F=p;
Adj_df=repmat(0,2,2);
F(1) = MSA/MSE; %IV F-statistic
F(2) = MSS/MSE; %within-subjects F-statistic

if v1>1 
    eps=GenCalcHFEps(X(:,1),[],X(:,2),X(:,3));
else
    eps=1;
end
