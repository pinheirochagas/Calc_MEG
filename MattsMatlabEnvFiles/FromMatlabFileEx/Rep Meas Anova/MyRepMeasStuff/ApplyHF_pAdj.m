function [p,eps,dfNum,dfErr] = ApplyHF_pAdj(X,F)
% p = ApplyHF_pAdj(X,F)
% 
% MJN: NOTE- this code is only valid if there are no b/t subj factors!
%
% I'm using this code to modify fxns I downloaded from teh cnetral file
% exchange (see below)... the difference b/t this and the original writers
% intent is that it applies the HF p-value correction automaticall, and
% returns the adjusted p-value.
%
% Inputs:
%    X - Input matrix can be a data matrix (size n-data x k-treatments)   
%       NOTE- if you have multiple factors in your anova design, average across
%       the other factor to get ecah value for ecah subject and treatment
%       of interest for this p-val adjustment.
%
%    F - Observed (calculated) F statistic value
% Output:
%    p - adjusted P-value.
%    dfNum - adjusted df for the numerator (the effect being observed)
%    dfErr - adjusted df for the relavent error term (the denominator)
%
% written by MJN on 090129...
%
...
disp('Adjusting repeated measures df and p-val Huynh-Feldt method')
if nargin < 2,
...
eps = epsHF(X); %call to the m-file Huynh-Feldt epsilon
eps=min(eps,1);
dfNum = eps*(k-1);  %adjusted numerator degrees of freedom
dfErr = eps*(k-1)*(n-1);  %adjusted denominator degrees of freedom
p = 1 - fcdf(F,dfNum,dfErr);
