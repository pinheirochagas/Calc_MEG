...
if TwoSigsFlag
    ...
    if ~all(isnan(coh))
        ...
        if CorrectedRawValForm  %see the comment above where this is defined for an explanation
            ...
            coh=sqrt(tmpdf-2).*atanh( abs(coh) )-1./sqrt(tmpdf-2);   %according to Thompson and Chave, and Koopmans, this is approximately distributed as a standard Gausian
        ...
        %coh=coh.*exp(1i*tmpang);    %... I do this in case one wants to preserve the angle in the output... though you could just take the abs of the result anyway...
        %I don't think the above line should be done, because if the coh following z-scorization is negative, that will interefere with the angle, and then the resulting magnuitude one would later take outside of this program if interested in it...
        %... YES, these z-scores can be negative...
        
        if CalcPart && ~JustPartFlag
            tmpang=angle(Pcoh);
            if CorrectedRawValForm  %see the comment above where this is defined for an explanation
                Pcoh=tanh( atanh( abs(Pcoh) )-1./(tmpdf-2) );
            else
                Pcoh=sqrt(tmpdf-2).*atanh( abs(Pcoh) )-1./sqrt(tmpdf-2);
            end
