function err=AcrossSessConds1DShuffTest_AllFreqBinSizes_tmp (TestStats1,TestStats2,f,ClusShuffOpts,pval)
...
MaxSums=repmat(0,[nShuffs, nrecf]);
tic
for iS=1:nShuffs
...
err.BestPValsPerBW=repmat(NaN,nrecf,1);
err.BestFirstInd=err.BestPValsPerBW;
err.AllPValsPerBW=cell(nrecf,1);
for BWdth=1:nrecf
    nRegs=nrecf - (BWdth-1);
    
    err.AllPValsPerBW{BWdth}=repmat(NaN,nRegs,1);
    for ix=1:nRegs
        csum=abs( sum( MnDiffs(ix:ix+BWdth-1) ) );     %assume no nansums to save time    
        err.AllPValsPerBW{BWdth}(ix)= sum( MaxSums(:,BWdth)>=csum )/nShuffs;
    end
                
    [vals, I]=sort( err.AllPValsPerBW{BWdth},1,'ascend');
    err.BestPValsPerBW(BWdth)=vals(1);
