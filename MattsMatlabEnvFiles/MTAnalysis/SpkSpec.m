function [Sx,f,tout,PSx,N,dn,nwin,err,tmpdf1] = SpkSpec(Spk,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts)
...
%               
%	    BN      =   The stop and start times (IN MS) to include as data. 
%                   Timestamps outside these times will be excluded. 
...
%ClusShuffOpts.CohAdjMag=0;  %b/c this is a spec fxn not a coh fxn, and we definitely don't want to do this for spec fxns   
%ClusShuffOpts.CohAdjPh=0;
%%%we don't need to do the two above lines of code b/c the shuffling was already written to only do the ph and mag adjustment if there are two signals present (i.e. it's called by a coh fxn)    
