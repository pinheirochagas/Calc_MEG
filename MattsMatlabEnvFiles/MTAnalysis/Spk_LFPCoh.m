...
%                   leads Y (meaning that the peak of the spike oscillation
%                   happens during the rising phase of the LFP oscillation,
%                   i.e. after the trough, but before the peak) while an 
%                   angle b/t Pi and 2Pi means Y leads Spk (and that the
%                   peak of the spike oscillation happens during the 
%                   falling phase of the LFP oscillation, i.e. after the
%                   peak btu before the trough)
%
...
if ~exist('bn','var') || isempty(bn);  
    bn=[t0 t0+size(Y,2)*1000/sampling-1]; %this assumes 1kHz sampling
end
if length(fk) == 1
    fk = [0,fk];
end
...
%note: N = number of samples at this point- not an absolute time
if pad==0
    nf=N;
else
