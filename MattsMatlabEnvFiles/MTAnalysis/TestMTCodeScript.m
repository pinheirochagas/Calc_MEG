...
options.normfreqs=0;
options.pad=1;
options.JustPartFlag=0;
options.errType=2;
options.ZTransFlag=2;
...
for it=1:nTr
    ...
    X(it,:)=RAmp*cos(2*pi*t/FreqPer-phi1) + 10.01*randn(1,nt);
    Y(it,:)=RAmp*cos(2*pi*t/FreqPer2-phi2) + .1*randn(1,nt);       
...
%test to see FX of freq. residual vs time residual for sine wave plus noise... to do this I stopped it during the function below   
figure
subplot(211)
plot(mean(Y))
subplot(212)
...
%I really don't even need to test this... it's obvious that with a pure
%sine wave, temporal and fourier comp averages will be the same...
%averaging the fourier components at a given frequency can just be thought of as filtering
%before averaging
...
%[coh,f,tout,Pcoh,Sx,Sy,PSx,PSy,N,dn,nwin,err,tmpdf1,tmpdf2]
%[coh,Myf,tout,PCoh,MyS1,MyS2,MyPS1,MyPS2,N,dn,nwin,err] = Spk_SpkCoh(Spk,Spk2,options);
[coh,Myf,tout,PCoh,MyS1,MyS2,MyPS1,MyPS2,N,dn,nwin] = Spk_SpkCoh(Spk,Spk2,options);

return

[~,Myf,~, Sx, Sy] = LFP_LFPCoh(X,Y,[],1000, [], [500], 0,[],[],4);

%[coh, f, tout, Sx, Sy, Pcoh, PSx, PSy, N, dn, nwin, errSF]=Spk_LFPCoh(Spk,X,options);
[coh, f, tout]=Spk_LFPCoh(Spk,X);%,options);    
[coh2, f2, tout2]=Spk_LFPCoh(Spk,-X);   %to tets teh effect of negating LFPs on Spk_Fld coherence

coh=coh(~isnan(coh));
coh2=coh2(~isnan(coh2));

PrecMarg=1e-5;

cohang=angle(coh)*180/pi;
cohang2=angle(coh2)*180/pi;
cohang3=cohang+180;
cohang3(cohang3>180)=cohang3(cohang3>180)-360;

cohmag=abs(coh);
cohmag2=abs(coh2);

numspc=numel(coh);
numeqph=sum(sum( abs(cohang3-cohang2)<PrecMarg ));
numeqmag=sum(sum( abs(cohmag2-cohmag)<PrecMarg ));

disp( [num2str(numeqph) ' out of ' num2str(numspc) ' spots have phases off by exactly 180 degrees.'] )
disp( [num2str(numeqmag) ' out of ' num2str(numspc) ' spots have the same coherence magnitude.'] )
%PostHocCohAngPlot(coh,f,tout);
return
...
PostHocSpecPlot(Sx,Myf,tout,'Sx')
PostHocSpecPlot(Sy,Myf,tout,'Sy')
