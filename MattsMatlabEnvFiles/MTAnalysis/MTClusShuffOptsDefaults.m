...
if ~isfield(ClusShuffOpts,'TestType');          ClusShuffOpts.TestType=1;   end   %vs. Base test ... set to 2 for a TrType test ... set to 3 for a Vs Null COh Test (shuffles trials... not valid for spec Power tests)   
if ~isfield(ClusShuffOpts,'TrType');            ClusShuffOpts.TrType=[];    end

if ~isfield(ClusShuffOpts,'CohAdjPh');          ClusShuffOpts.CohAdjPh=1;   end   
