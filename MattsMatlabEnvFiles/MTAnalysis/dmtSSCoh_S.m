...
if detrendflag
    ...
    %tmp2=detrend(tmp2);
    tmp=detrend(tmp,2);   %detrends the rows of tmp (which are ecah one trial of X)
    tmp2=detrend(tmp2,2);
end     
%tmpmn=mean(tmp);    %tmpmn will be a row vec with the mean val for each tr
%tmpmn2=mean(tmp2);
tmpmn=mean(tmp,2);    %tmpmn will be a column vec with the mean val for each tr
tmpmn2=mean(tmp2,2);

tmpInds=GenSecDimVarInds(K,nch);
tmpTaps=tapers(repmat([1:K]',nch,1),:);

Xk = fft( tmpTaps.*tmp1( tmpInds,: ),nf,2 )-H(repmat(1:K,1,nch),:).*repmat( tmpmn( tmpInds,1 ),1,nf );  %with an FFT, freq samples replace time samples for each trial, regardless of the dimension the FFT was performed on...      
Yk = fft( tmpTaps.*tmp2( tmpInds,: ),nf,2 )-H(repmat(1:K,1,nch),:).*repmat( tmpmn2( tmpInds,1 ),1,nf );  %with an FFT, freq samples replace time samples for each trial, regardless of the dimension the FFT was performed on... so this outputs
