function err=AcrossSessConds1DShuffTest_AllFreqBinSizes (TestStats1,TestStats2,f,ClusShuffOpts,alpha)
...
tic
for iS=1:nShuffs
    if mod(iS,5)==0;      disp(['OnShuff:' num2str(iS)]);     end
...
for BWdth=1:nrecf
    ...
    for ix=1:nRegs
        ...
        [~, err.AllPoolTtestPValsPerBW{BWdth}(ix)]= ttest2( Act1All( nReps1*(ix-1)+1:nReps1*(ix-1+BWdth) ),Act2All( nReps2*(ix-1)+1:nReps2*(ix-1+BWdth) ) );    %using a 1 dimensional indexing of the Act1All and Act2All arrays
...
%Note- for general usefulness of this, we'll find the best time window, regardless of the p-value being below a certain threshhold, and output both the best location an dthe best p-value   
