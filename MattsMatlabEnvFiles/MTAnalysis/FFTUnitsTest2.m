%FFTUnitsTest2.m
% Created by adjusting FFTUnitsTest on 090226, with the intentino of
% double-checking that the units of LFPSPec indeed properly result in the
% PSD...
%
...
MatlabCheck=0;
PerCheck=0;
esttype='ms';   %'psd' or 'ms' ... for Periodogram

DblChronFlag=1; %doubels the Chronux value to correct it... this makes it line up almost perfectly with the matlab value...
plotflag=1;

% Sampling frequency
Fs = 1000;
%d=1; %sec... duration of signal
zpad=4;
...
ncycs=50;
d=ncycs/sigf;  %set duration of signal to generate ncycs worth of data...
...
otapers=[d,P*1/d,2*P-1];    %This is set to have P=3, not W=3
W=otapers(2);
tapers=otapers;
ntapers=otapers(3);
...
if MTCheck
    ...
    [spec,F]=LFPSpec(x,tapers,Fs, d, Fs/2,0,[],zpad);
    
    AmpSpecMT=Pow2Amp(spec);
    if plotflag                
        %plot(F,spec,'-x','Color',mycolors(2));        
        plot(F,AmpSpecMT,'-x','Color',mycolors(2)); 
        hold on
    end
end
...
if ChronCheck
    ...
    if DblChronFlag
        S=2*S;  %this doubles the Chron val to account for their error
    end
    
    if plotflag        
        plot(fChron,S,'-x','Color',mycolors(3));
        hold on
    end
end
...
if MatlabCheck
    ...
    if plotflag        
        plot(fMatlab,Pxx,'-x','Color',mycolors(4));
        hold on
    end
end
...
if PerCheck
    ...
    [PxxPer,fPer] = periodogram(x,win,NFFT,Fs,esttype);
    
    AmpSpec=Pow2Amp(PxxPer);
    if plotflag        
        %plot(fPer,PxxPer,'-x','Color',mycolors(5));
        plot(fPer,AmpSpec,'-x','Color',mycolors(5));
        hold on
    end
end
