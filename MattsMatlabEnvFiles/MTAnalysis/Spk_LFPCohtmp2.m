...
if nargin<15 || isempty(ClusShuffOpts)
    %instead of writing these defaults to ClusShuffOpts, add them directly
    %to the workspace...
    nShuffs=5000;
    ClusShuffOpts.BasePer=[-320 -80]; %in ms- this is the time window corresponding to the baseline period. A given window must be ENTIRELY confined to this interval to be considered to be in the baseline period
    ClusShuffOpts.ActPer=[0 800]; %in ms- this is the time window corresponding to the activity period, which are all the time periods you wish to test against the baseline period. A given window must be ENTIRELY confined to this interval to be considered to be in the activity period
    ClusShuffOpts.Thresh=1.96;
    ClusShuffOpts.NClusCutOff=6;    %To improve speed, the algorithm operates two different ways (with the same result) based on the number of clusters in a given shuffle- one way is faster if there are more clusters and one way is faster if there are fewer clusters. This value sets the cutoff number of clusters value to determine which algorithm to use on a particular shuffle   
    ...
    ClusShuffOpts.SpecTestStat=1;   %1 to use a sign-rank test test stat, 2 touse the log-normalized differences as the test stat
    ...
    ClusShuffOpts.TestType=1;   %vs. Base test ... set to 2 for a TrType test
