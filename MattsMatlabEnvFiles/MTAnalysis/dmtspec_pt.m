...
%
   for ch = 1:nch
	...
	dNk = fft(ntapers(:,1:K).*tmp(:,ones(1,K)),nf)'...
        ...
        Sk = abs(dNk).^2;
        spec(ch,:) = mean(Sk,1);    %averaging across tapers
  	rate(ch) = mean(sum((ntapers(:,1:K).*tmp(:,ones(1,K))).^2,1));
        ...
        if errorchk	%  Estimate error bars using Jacknife
          for ik = 1:K  %for each taper
            ...
            dNj = dNk(indices,:);
            jlsp(ik,:) = log(mean(abs(dNj).^2,1)); %a jackknife sample in each row leaving a different taper out, with a different frequency in each column
                    %note using the log of the spectrum for statistics,
                    %probably because it stabilizes the variance
          end
          lsig = sqrt(K-1).*std(jlsp,1);    %I'm not sure about the rest, or how they know it follows a t-distribution; this may be worth double checking...
          crit = tinv(1-pval./2,dof-1);	%   Determine the critical factor
          err(1,ch,:) = exp(log(spec(ch,:))+crit.*lsig);
          err(2,ch,:) = exp(log(spec(ch,:))-crit.*lsig);     %converts back to non-log units
         end
    end
end
...
if flag			% Pooling across trials
   ...
   spec = mean(abs(dNk).^2,1);  %averaging across tapers and trials
   rate = rate./nch;
   ...
   if errorchk		%  Estimate error bars using Jacknife
	for ik = 1:nch*K    %for each taper/trial combination
	   ...
	   dNj = dNk(indices,:);
	   jlsp(ik,:) = log(mean(abs(dNj).^2,1));   %a jackknife sample in each row leaving a different taper/trial comb out, with a different frequency in each column
                   %note using the log of the spectrum for statistics,
                   %probably because it stabilizes the variance
	end
    lsig = sqrt(nch*K-1).*std(jlsp,1);     %I'm not sure about the rest, or how they know it follows a t-distribution; this may be worth double checking...
	crit = tinv(1-pval./2,dof-1);	%   Determine the scaling factor
    err(1,:) = exp(log(spec)+crit.*lsig);
    err(2,:) = exp(log(spec)-crit.*lsig);   %converts back to non-log units
   end
end
