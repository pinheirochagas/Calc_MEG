...
if ~DontTakeAbsVal  %the UseRawCohAvgStats option d/n exist for this program yet, and it's not clear that it really does need to be written, at least not at this moment     
    TestStats1=abs(TestStats1);
    TestStats2=abs(TestStats2);
end
...
for iS=1:nShuffs    
    ...
    if UseTTest
        for iF=1:nrecf
            for iW=1:nInActPer
                [~,p]=ttest( squeeze( ShuffAct1(iW,iF,:) ),squeeze( ShuffAct2(iW,iF,:) ) );
                curTStat(iF,iW)=norminv(1-p/2,0,1);   %here the resulting curTStat will only be positive
                if mean(ShuffAct2(iW,iF,:)) > mean( ShuffAct1(iW,iF,:) )
...
if UseTTest
    for iF=1:nrecf
        for iW=1:nInActPer
            ...
            [~,p]=ttest( squeeze( Act1All(iW,iF,:) ),squeeze( Act2All(iW,iF,:) ) );
            curTStat(iF,iW)=norminv(1-p/2,0,1);   %here the resulting curTStat will only be positive
            if mean(Act2All(iW,iF,:)) > mean( Act1All(iW,iF,:) )
                curTStat(iF,iW)=-curTStat(iF,iW);   %here we turn the given curTStat negative if needed
            end
        end
    end    
