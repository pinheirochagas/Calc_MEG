function [coh,f,tout,Pcoh,Sx,Sy,PSx,PSy,N,dn,nwin,err,tmpdf1,tmpdf2] = Spk_SpkCoh(Spk,Spk2,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts)
...
%   types of trials. [note can also set to 3 for a VsNullCoh test... see below]  
%   When using TestType of 2, the user must also input
...
%   altering the shuffled coherenec and thus potentially altering the
...
%
%   Added on 100208- The paramater ClusShuffOpts.TrType can be set
%   to 3 to do trial shuffling to show the existence of significant
%   coherence vs the null hypothesis of no coherence. Note that this only
%   works for coherence, but is not valid for spetcral power (thus the code
%   won't perform the test for spectral power). This will only be done for
%   time windows within the ActPer... Also note that this test does not 
%   need the Mag or Ph Adj., so that will not be done for this regardless
%   of the vals of ClusShuffOpts.CohAdjPh, etc.
%
%   INPUTS: 
...
FSPadFlag=0;    % Set to 1 if you want the number of points in the FFT to be a the next power of 2 "relative to multiple of the Sampling rate in Hz rather than always a power of 2. 
                % The advantage of this is that the specific frequency measurements you make will be done at more round numbers. (relative to your sampling rate)      
                % The disadvantage is that if Fs is not a power of 2, this means the FFT algorithm will run more slowly
SpecPadToVal=0; % If this is anything but zero, the program will zero pad the fft to this particular value, no matter what the value is    
                % This is useful for example for 1D analyses with sessions with different lengths where you want to pad them to all have the same equivalent lengths and have teh frequency measurements done at the same precise values   
                % The units of this should be in #'s of samples    
...
%need to add this to other primary MT programs after I verify that it works
SetOutputRes=0;  %param created for KassLabCoh anal... set to greater than 0 to adjust the freq resolution of the result before doing shuffling...      
    ...
    for ifld=1:length(fieldlist)
        ...
        if strcmp(fieldlist{ifld},'ClusShuffOpts')
            ...
            if length(options.(fieldlist{ifld}))>1
                ...
                eval([fieldlist{ifld} '= options.(fieldlist{ifld});']);
            else
                eval([fieldlist{ifld} '=' num2str(options.(fieldlist{ifld})) ';']);
            end
        end
...
if length(tapers) == 2
    ...
    %adjust n + w in case k is zero b/c of shitty rounding error
    if k==0 && n*w+.001>1
        w=w+.001;
        k = floor(2*n*w-1);
    end
...
if length(tapers) == 3
    ...
    if tapers(3)> floor(2*tapers(1)*tapers(2)-1+1e-4) %this adjusts k automatically and notifies the user if they enter a value for k larger than the usable amount 
    ...
    %tmp=toc;
    %disp(['Done calcing tapers; ' num2str(tmp) ' seconds elapsed.'])
...
% Determine outputs
if nargout > 3 || JustPartFlag;     CalcPart = 1;    else        CalcPart = 0;       end
if nargout > 11; errorchk = 1;    else        errorchk = 0;       end
if nargout > 4; CalcSpec = 1;    else        CalcSpec = 0;       end
...
if NoPadForClusShuff && errorchk && errType==2       
    ...
    %
    %Added later on 100210: My understanding of what I was thinking is that this is a problem only in conjunction w/ the clustering not the shuffling, b/c w/ the clustering w/ zero padding, clusters will tend to gather b/c nearby samples are not independent... so this unfairly exacerbates differences between teh actual and shuffled daata on a cluster level... but a vs max test should still be OK (or close enough to OK) I think      
    %And this would apply to all types of tests with clustering: vs Baes, vsCond, and even all AcrossSess tests...   
end

if nargout > 5 && JustPartFlag;
    disp('In Spk_Spk Coh., calcing just the partial coh, given in first output (coh). 4th output (PCoh) will be empty.')
    Pcoh=[];
end
...
if pad==0 && SpecPadToVal <= 0
    nf=N;
else
    if SpecPadToVal>0;   nf = SpecPadToVal;
    elseif FSPadFlag;    nf = sampling*pad*2^nextpow2(N/sampling);  %nf = max(sampling*2^nextpow2(256/sampling), sampling*pad*2^nextpow2(N/sampling));
...
if errorchk && errType==2   
    if isstr(ClusShuffOpts.TestType)
        ...
        elseif ClusShuffOpts.TestType=='vsNullCoh';    ClusShuffOpts.TestType=3;
        else
            errorchk=false;
...
if SetOutputRes>0
    fraw=f;
    %[OutVals fout]=AvgAcrossFreqs(InVals,fraw,fstep,fk,f0cut,CalcType)
    [cohmag f]=AvgAcrossFreqs(abs(coh),fraw,SetOutputRes,fk,[],1);
    cohang=AvgAcrossFreqs(coh,fraw,SetOutputRes,fk,[],4);
    coh=cohmag.*exp(1i*cohang*pi/180);
    nrecf=length(f);
    
    if CalcPart && ~JustPartFlag
        Pcohmag=AvgAcrossFreqs(abs(Pcoh),fraw,SetOutputRes,fk,[],1);
        Pcohang=AvgAcrossFreqs(Pcoh,fraw,SetOutputRes,fk,[],4);
        Pcoh=Pcohmag.*exp(1i*Pcohang*pi/180);
    end
    if CalcSpec
        Sx=AvgAcrossFreqs(Sx,fraw,SetOutputRes,fk,[],1);
        Sy=AvgAcrossFreqs(Sy,fraw,SetOutputRes,fk,[],1);
        if CalcPartSpec && ~JustPartFlag
            PSx=AvgAcrossFreqs(PSx,fraw,SetOutputRes,fk,[],1);
            PSy=AvgAcrossFreqs(PSy,fraw,SetOutputRes,fk,[],1);
        end
    end        
end
...
if errorchk 
    if SplitTrsFlag
        MTShuffCondsTest_S
    elseif vsNullCohFlag
        MTShuffVsNullCohTest_S
    else
