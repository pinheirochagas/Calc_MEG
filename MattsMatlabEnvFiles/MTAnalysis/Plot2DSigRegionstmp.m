function Plot2DSigRegions( SigMat,axh,sigt,sigf,plott,plotf,LW,VOffset,HOffset,VExtraLen,HExtraLen ) 
%function Plot2DSigRegions( SigMat,axh,sigt,sigf,plott,plotf,LW,VOffset,HOffset,VExtraLen,HExtraLen ) 
%
...
%
% INPUTS
%   SigMat:         A matrix of 0's and 1's [Time x Freq], 0's referring to
%                   wherever the region tested for significance was not 
%                   significant, and 1 referring to when it was.
...
if nargin<10 || isempty(HExtraLen);    HExtraLen=0.07;    end
if nargin<10 || isempty(VExtraLen);    VExtraLen=0.07;    end
if nargin<9 || isempty(HOffset);    HOffset=0.7;    end
if nargin<8 || isempty(VOffset);    VOffset=0.7;    end
if nargin<7 || isempty(LW);    LW=3;    end
...
SigMat=[repmat(0,nSt,1) SigMat repmat(0,nSt,1)];
SigMat=[repmat(0,1,nSf+2); SigMat; repmat(0,1,nSf+2)];
...
for iL=1:length(HLinIndsX)
    tmpy=max(HLinIndsY(iL)-VOffset,VOffset)+TAdj;    %the limit is needed for the limiting case of the first row, so that the line is created off the limits of the axis... instead it is drawn as if it were "going the other way" for the row below it...     
    plot( [HLinIndsX(iL)-0.5-HExtraLen HLinIndsX(iL)+0.5+HExtraLen]-1+FAdj,[tmpy tmpy],'k','LineWidth',LW )    %the extra -1 is to account for the zeros added around SigMat   
...
for iL=1:length(HLinIndsX)
    ...
    tmpy=min(HLinIndsY(iL)+VOffset-1, length(plotf)+1-VOffset)+TAdj;    %the limit is needed for the limiting case of the first row, so that the line is created off teh limits of the axis... instead it is drawn as if it were "going the other way" for the row below it...     
    plot( [HLinIndsX(iL)-0.5-HExtraLen HLinIndsX(iL)+0.5+HExtraLen]-1+FAdj,[tmpy tmpy],'k','LineWidth',LW )    
end
...
for iL=1:length(VLinIndsX)
    tmpx=max(VLinIndsX(iL)-HOffset,HOffset)+FAdj;    %the limit is needed for the limiting case of the first row, so that the line is created off teh limits of the axis... instead it is drawn as if it were :going the other way" for the row below it...     
    plot( [tmpx tmpx],[VLinIndsY(iL)-0.5-VExtraLen VLinIndsY(iL)+0.5+VExtraLen]-1+TAdj,'k','LineWidth',LW )    
end

[VLinIndsY VLinIndsX]=find( VdSigMat==-1 );  %these will serve as the cells with 
for iL=1:length(VLinIndsX)
    %here we do need to account for the added edge zeros in the diffed dimension too     
    tmpx=min(VLinIndsX(iL)+HOffset-1,length(plott)+1-HOffset)+FAdj;    %the limit is needed for the limiting case of the first row, so that the line is created off teh limits of the axis... instead it is drawn as if it were :going the other way" for the row below it...     
    plot( [tmpx tmpx],[VLinIndsY(iL)-0.5-VExtraLen VLinIndsY(iL)+0.5+VExtraLen]-1+TAdj,'k','LineWidth',LW )    
end
