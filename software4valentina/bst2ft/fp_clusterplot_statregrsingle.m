function fp_clusterplot_statregrsingle(cfg,stat,sensortype,varargin)
...
datadiff=ft_selectdata(varargin{1},'toilim',[stat.time(1) stat.time(end)]);
...
% define time window
srate=round(1/(varargin{1}.time(2)-varargin{1}.time(1)));
...
if ~isfield(cfg, 'zlim')
    ind_timewin_data=round(ns_lat2point([cfg.lat(1) cfg.lat(end)],1,srate,[varargin{1}.time(1) varargin{1}.time(end)],1));
    for level=1:nl
            max_loc_loc = varargin{level}.avg(:,ind_timewin_data(1):ind_timewin_data(end));
            max_loc(level)=max(abs(max_loc_loc(:)));
    end;
    maxlim=max(max_loc);
...
%         ft_topoplotTFR(cfgtopo, data1);
    ...
    if Nsigall == 0
        ...
        for frame=1:nframes
            ...
            for level=1:nl
                subplot(1,nl+1,level);
                ft_topoplotTFR(cfgtopo, varargin{level});
            end;
            subplot(1,nl+1,nl+1);
