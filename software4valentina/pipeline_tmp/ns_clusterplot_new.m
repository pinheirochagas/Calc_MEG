function ns_clusterplot(cfg,stat,data1,data2)
% function ns_clusterplot(cfg,stat,data1,data2) plots a series of topoplots from averaged data from condition 1 (first row, data1), 
...
%        stat=stat{3};
    case 'grad_cmb'
        data1 = ft_selectdata(data1,'channel',data1.typelabel.Grad2_cmb);
        data2 = ft_selectdata(data2,'channel',data1.typelabel.Grad2_cmb);
    otherwise
...
datadiff=data1;
datadiff.avg=data1.avg-data2.avg;
...
if ~isfield(cfg, 'zlim')
    ...
    maxlim=max(abs([data1loc(:); data2loc(:)]));
...
if haspos == 0 && hasneg == 0
    ...
    for frame=1:nframes
        ...
        subplot(3,nframes,frame);
        ft_topoplotTFR(cfgtopo, data1);
        cfgtopo.comment='no';
        subplot(3,nframes,nframes+frame);
        ft_topoplotTFR(cfgtopo, data2);
        subplot(3,nframes,2*nframes+frame);
        cfgtopo.highlightchannel = [];
