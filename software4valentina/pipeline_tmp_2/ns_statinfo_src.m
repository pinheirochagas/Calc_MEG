function [roipos,roineg]=ns_statinfo_src(varargin)
...
if length(varargin)==1 
    probthr=0.9; % threshold of max P-value of displayed cluster
else
    probthr=varargin{2};
end;
        ...
        if stat.posclusters(1).prob>probthr
            ...
            for i=1:nposclusters
                if stat.posclusters(i).prob<=probthr
                    ...
                    a=stat.Time(clus>0);
                    tp=mean((stat.posclusterslabelmat==i).*stat.tmap,1);
                    [mx,tmx]=max(tp);
        ...
        if stat.negclusters(1).prob>probthr
            ...
            for i=1:nnegclusters
                if stat.negclusters(i).prob<=probthr
                    ...
                    a=stat.Time(clus>0);
                    tp=mean((stat.negclusterslabelmat==i).*stat.tmap,1);
                    [mx,tmx]=min(tp);
