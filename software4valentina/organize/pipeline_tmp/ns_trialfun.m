function trl = ns_trialfun(cfg)
% function trl = ns_trialfun(cfg)
% determines trials/segments in the data that are
% interesting for analysis, using the general event structure returned
% by read_event. This function is made for Neuromag data from Neurospin and
% is used by ft_definetrial in ns_fif2ft to define the beginning and end of
% each epoch in the data.
%
% Trigger times and values are computed from the difference between
% consecutive stimulus channel values (only positive differences are
% considered, corresponding to positive jumps in the channel values).
%
% The ouput trl contains the beginning and end sample, the pretrigger 
% time window and the value of the trigger for each
% epoch.
%
% NOTE: This is the function that incorporates the stimulus-trigger delay.
