...
def pretty_gat(scores, times=None, chance=0, ax=None, sig=None, cmap='RdBu_r',
               ...
               test_times=None, smoothWindow=0, classLines=None, classColors=None):
    ...
    if diagonal is not None:
        ax.plot([np.max([min(times), min(test_times)]),
                ...
                [np.max([min(times), min(test_times)]),
                 np.min([max(times), max(test_times)])], color=diagonal, linestyle = 'dashed', linewidth = 0.4)
    # setup ticks
...
def pretty_decod(scores, times=None, chance=0, ax=None, sig=None, width=3.,
                 color='k', fill=False, xlabel='Times', sfreq=250, alpha=2, scat=False, line=True, smoothWindow=0, lim=None):
    scores = np.array(scores)
	...
	if scores.ndim == 2:
        ...
        if smoothWindow > 0:
			...
			plot_sem(times, scores_m, sem, color=color, ax=ax, line_args = {'linewidth': 2})
        elif smoothWindow == 0:
			plot_sem(times, scores, [], color=color, ax=ax, line_args = {'linewidth': 2})
    else:
        scores_m = scores
    ...
    if sig is not None:
        ...
        if fill:
            ...
            plt.scatter(times_sig, scores_sig, s=20, color=color)
        elif line: #Added by Darinka Feb 9th 2016 to plot significance as separate horizontal line
            times_sig = np.intersect1d(times[sig < .05], times[scores_m > .5]) #select all those time points in which the significance is less than .05 & the accuracy above .5
            plt.scatter(times_sig, np.repeat(min(scores_m - sem)+0.004, len(times_sig)), s=4, color=color)
        else:
    ...
    ax.set_yticklabels(['%.2f' % ymin, 'Chance', '%.2f' % ymax], fontdict={'fontname': 'Times New Roman', 'fontsize': 14})
