...
def smooth(data, window):
	...
	#elif data.ndim == 2: #Gat matrix
		#for t in range(data.shape[0]): #loop through the first dimension
			#if t <= window:
				#data[t, :] = np.mean(data[t : (t + window), :], 0)
			#elif t >= data.shape[0] - window:
				#data[t, :] = np.mean(data[(t - window) : t, :], 0)
			#else:
				#data[t, :] = np.mean(data[(t - window) : (t + window), :], 0)
		
		#for t in range(data.shape[1]): #loop through the second dimension
			#if t <= window:
				#data[:, t] = np.mean(data[:, t : (t + window)], 1)
			#elif t >= data.shape[0] - window:
				#data[:, t] = np.mean(data[:, (t - window) : t], 1)
