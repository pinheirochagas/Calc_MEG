...
from calc_ClassifyLinearReg import calc_ClassifyLinearReg
...
for sub in subjects:
    ...
    idx_trainSet = (info['preResult'] >= 3) & (info['preResult'] <= 6) 
    idx_testSet =  (info['preResult'] >= 3) & (info['preResult'] <= 6) 
    ...
    gat, score, diagonal = calc_ClassifyLinearReg(X_train, y_train, [], [], params)
...
fname = op.join(result_path, 'Regression_ ' + '_Trainset_' + trainset + '_Testset' + testset) 
