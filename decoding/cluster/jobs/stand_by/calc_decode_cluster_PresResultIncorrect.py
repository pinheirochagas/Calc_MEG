...
from calc_multiClassClassify import calc_multiClassClassify
...
#Decoding
trainset = 'presResInc'
testset = 'presResInCor'
decCond = ['presResInCor']
...
for sub in subjects:
    ...
    idx_trainSet = (info['absdeviant'] != 0) & (info['preResult'] >= 3) & (info['preResult'] <= 6) 
    idx_testSet = (info['absdeviant'] != 0) & (info['preResult'] >= 3) & (info['preResult'] <= 6) 
    info_train = info[idx_trainSet]
    info_test = info[idx_testSet]

    # Correct the cue labels
    labels_train = 'preResult'
    ...
    # Crop train epochs
    X_train = X_train.crop(tmin=X_train.times[75], tmax=X_train.times[-1]) # crop from -200 on
    ...
    gat, score, diagonal = calc_multiClassClassify(X_train, y_train, [], [], params)
...
fname = op.join(result_path, 'Classification_ ' + '_Trainset_' + trainset + '_Testset' + testset) 
np.save(fname, all_scores)
fname = op.join(result_path, 'ResInc_train_times') 
np.save(fname, X_train.times)
fname = op.join(result_path, 'ResInc_test_times') 
